...
The chunk below creates virtual environment and installs python libraries in there. It is set to eval=FALSE since one should only run it once.
```{r create venv and install libraries, eval=FALSE}
virtualenv_create(envname = "RLadies", packages = c("pandas", "pydeseq2"), python = "3.12")
```
